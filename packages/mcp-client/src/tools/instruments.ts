...
export function registerInstrumentTools(server: McpServer, api: PrrmApiClient) {
  ...
  server.tool(
    ...
    "Create a new instrument in the system",
    {
      ...
      assetClass: z.enum(["equity", "fixed_income", "commodity", "fx", "crypto", "other"]).describe("Asset class"),
  ...
  server.tool(
    ...
    "Update fields on an existing instrument",
    {
      ...
      assetClass: z.enum(["equity", "fixed_income", "commodity", "fx", "crypto", "other"]).optional().describe("Updated asset class"),
